...
def Update_Code():
    ...
    def Bias_Monsters_Calculation(num_Monsters,Bias_Monsters):
        ...
        for i in M:
            ...
            for k in range(M[i].Number.value):
                B_M_Damage.append(P_BH*float(M[i].Damage.value)/total_Damage)
                B_M_HP.append(int(M[i].HP.value)/(P_TH*total_HP))
...
class Monster:
    def __init__(self,n):
        ...
        self.custom = tk.Button(self.fr, text="Auto", width = 8, command=lambda f=n: Custom(f))
...
def Custom(n):
    ...
    M[n].Existing = tk.Button(M[n].fr, text="Manual", width = 8, command=lambda f=n: Exising(f))
...
def Exising(n):
    ...
    M[n].fr_custom.grid_forget()
    M[n].Custom = tk.Button(M[n].fr, text="Auto", width = 8, command=lambda f=n: Custom(f))
